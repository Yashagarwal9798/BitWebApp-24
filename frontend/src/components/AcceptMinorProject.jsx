...
const AcceptMinorProject = () => {
  ...
  const handleAcceptGroup = async (groupId) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.post("/api/v1/prof/minor/accept-group", {
          _id: groupId,
        });
        toast.success(response.data.message);
  ...
  const handleDenyGroup = async (groupId) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.post("/api/v1/prof/minor/deny-group", {
          _id: groupId,
        });
        toast.success(response.data.message);
        if (response.status === 200) window.location.reload();
  ...
  const handleMergeGroups = async () => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await axios.post("/api/v1/prof/minor/merge-groups", {
                                ...
                                {viewMode === "accepted" && (
                                  <td className="px-6 py-4 whitespace-nowrap">
                                    {showMarksInputFor === member._id ? (
                                      ...
                                      >
                                        <span>
                                          {member.marks.minorProject || 0}/50
                                        </span>
